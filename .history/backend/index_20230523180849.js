...
const ellipticCurve = new elliptic.ec.ec('secp256k1');
const app = express();
app.use(cors());
...
app.post("/decrypt-with-ecc", (req, res) => {
  ...
  const decryptedData = ellipticCurve.keyFromPrivate(privateKey, 'hex').decrypt(Buffer.from(ciphertext, 'hex')).toString();

  res.send({ decryptedPlaintext: decryptedData });

...
app.post("/encrypt-with-ecc", (req, res) => {
  const plaintext = req.body.plaintext;
  const eccKeyPair = new sjcl.ecc.elGamal.generateKeys(256);
  const publicKey = sjcl.codec.hex.fromBits(eccKeyPair.pub.get());
  const privateKey = sjcl.codec.hex.fromBits(eccKeyPair.sec.get());
  const ciphertext = sjcl.encrypt(publicKey, plaintext);

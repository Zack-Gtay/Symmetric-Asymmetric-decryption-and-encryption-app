...
app.post('/encrypt', (req, res) => {
  ...
  const publicKey = keyPair.getPublic('hex');
  const privateKey = keyPair.getPrivate('hex');
  const ciphertext = ec.keyFromPublic(publicKey, 'hex').encrypt(plaintext).toString();
  res.send({ publicKey: publicKey, privateKey: privateKey, ciphertext: ciphertext });
});

app.post('/decrypt', (req, res) => {
  const encrypted = req.body.encrypted;
  const privateKey = req.body.privateKey;
  const plaintext = ec.keyFromPrivate(privateKey, 'hex').decrypt(encrypted).toString();
  res.send({ plaintext: plaintext });
});

...
app.post("/encrypt-with-ecc", (req, res) => {
  ...
  const publicKeyHex = eccrypto.bytesToHex(publicKey);
  const privateKeyHex = eccrypto.bytesToHex(privateKey);
  ...
  Promise.all([publicKeyHex, privateKeyHex, ciphertextBuffer])
    .then(([publicKey, privateKey, ciphertext]) => {
      res.send({
        publicKey: publicKey,
        privateKey: privateKey,
        ciphertext: eccrypto.bytesToHex(ciphertext),
      });
    })
    .catch((error) => {
      console.error(error);

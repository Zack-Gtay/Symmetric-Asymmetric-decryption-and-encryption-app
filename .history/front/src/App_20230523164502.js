...
const App = () => {
  ...
  const handleEncrypt2 = () => {
    Axios.post('http://localhost:5000/encrypt-with-ecc', { publicKey: publicKey, plaintext: plaintext2 })
      .then(response => {
  ...
  const handleDecrypt2 = () => {
    Axios.post('http://localhost:5000/decrypt-with-ecc', { privateKey: privateKey, ciphertext: ciphertext })
      .then(response => {
        setDecryptedPlaintext(response.data.decryptedPlaintext);
      })
      .catch(error => {
        console.log(error);
      });
